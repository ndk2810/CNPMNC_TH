...
const addTour = (e) => {
    ...
    if (!DiaDiem || !TheLoai || !TenTour || !DiaChiTour || !DiemNoiBat || !LichTrinh || !NoiDung || !DoDai || !GiaNguoiLon || !GiaTreEm) {
...
const deleteTour = (IDTour) => {
    ...
    if (window.confirm('Xoá tour ?')) {
        axios.post(server + '/tour/remove', data)
...
const modalThemTour = () => {
    ...
    FormThemTour.style.display === 'block' ? FormThemTour.style.display = 'none' : FormThemTour.style.display = 'block'
}

const quanLyTour = ({ Tours, DiaDiems, ListTheLoaiTour }) => {
    return (
        <div>
            <button className="btn-modal-show" onClick={modalThemTour}>Thêm tour</button>
            <FormThemTour addTour={addTour} DiaDiems={DiaDiems} ListTheLoaiTour={ListTheLoaiTour} />
